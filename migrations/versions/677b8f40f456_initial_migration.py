...
def downgrade():
    ...
    sa.Column('productcategoryid', sa.SMALLINT(), server_default=sa.text("nextval('product_category_productcategoryid_seq'::regclass)"), autoincrement=True, nullable=False),
    ...
    sa.Column('productcategoryid', sa.SMALLINT(), server_default=sa.text("nextval('request_productcategoryid_seq'::regclass)"), autoincrement=True, nullable=False),
    sa.CheckConstraint("lender ~* '^[\\w\\-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$'::text", name='valid_lender_email'),
    sa.CheckConstraint("requester ~* '^[\\w\\-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$'::text", name='valid_requester_email'),
    sa.CheckConstraint('requester <> lender', name='valid_lender_user'),
    sa.ForeignKeyConstraint(['productcategoryid'], ['product_category.productcategoryid'], name='request_product_category_fk', onupdate='RESTRICT', ondelete='RESTRICT'),
